-- Echo Map — per-echo discover radius + search range support
-- Safe to re-run. Run in Supabase → SQL Editor after echo-aura / echoes-images patches.

alter table public.echoes
  add column if not exists discover_radius_m double precision not null default 420;

update public.echoes
set discover_radius_m = 800
where discover_radius_m < 420;

alter table public.echoes drop constraint if exists echoes_discover_radius_check;
alter table public.echoes add constraint echoes_discover_radius_check
  check (discover_radius_m >= 420 and discover_radius_m <= 2500);

-- Publish with discover radius (420m – city).
...
begin
  ...
  if radius_m < 420 or radius_m > 2500 then
    raise exception 'Discover radius must be between 420m and 2500m';
  end if;
  ...
  insert into public.echoes (
    ...
    label, city_label, expires_at, discover_radius_m
  )
  values (
...
returns table (
  ...
  discover_radius_m double precision,
  aura_count bigint,
  i_gave_aura boolean
)
language sql
