...
export function canHintEcho(echo, { followingIds }) {
  if (!echo || echo.mine) return false
  if (!ECHO_PUBLIC_VISIBILITIES.has(echo.visibility)) return false
  if (echo.visibility === 'friends' && !followingIds?.has(echo.ownerId)) return false
  return true
}

/** Whether proximity discovery / notifications should fire for this echo. */
export function canDiscoverEcho(echo, { followingIds }) {
  if (!echo || echo.mine) return false
  if (!ECHO_PUBLIC_VISIBILITIES.has(echo.visibility)) return false
  if (echo.visibility === 'friends' && !followingIds?.has(echo.ownerId)) return false
  return true
...
export function canOpenRemotely(echo, { discovered, followingIds }) {
  if (!echo) return false
  if (echo.mine) return true
