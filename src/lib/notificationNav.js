...
const ECHO_PLACE_KEY = 'frens-echo-explore-place'

/** Queue opening Echo map in explore mode for a place (from absolute search). */
export function requestEchoExplorePlace(place) {
  if (!place?.lat || !place?.lon) return
  try {
    sessionStorage.setItem(
      ECHO_PLACE_KEY,
      JSON.stringify({
        id: place.id || place.placeKey || null,
        label: place.label || place.placeLabel || place.cityLabel || 'Place',
...
export function consumeEchoExplorePlace() {
  try {
    const raw = sessionStorage.getItem(ECHO_PLACE_KEY)
    if (!raw) return null
    sessionStorage.removeItem(ECHO_PLACE_KEY)
    const parsed = JSON.parse(raw)
    if (!parsed?.lat || !parsed?.lon) return null
    return {
      id: parsed.id || null,
      label: parsed.label || 'Place',
