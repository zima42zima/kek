...
export default function SendOwlLetterModal({ recipient, onClose, onSent }) {
  ...
  return (
    <Modal
      title={<span className="inline-flex items-center gap-2"><OwlPostIcon className="w-5 h-5" /> Letter for {recipient.frenName}</span>}
      onClose={onClose}
      maxWidth="max-w-5xl"
    >
      {loading ? (
        ...
        <form onSubmit={handleSend} className="space-y-4 owl-letter-ui">
          ...
          <button
            ...
            className="w-full owl-btn-primary py-3 disabled:opacity-50"
          >
